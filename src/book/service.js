...
import { findAuthorByName, findAuthorsByid } from '../author/service'
import { findPublisherByName, findPublisher } from '../publisher/service'

var ObjectId = mongodb.ObjectId

export async function findBooks(args) {
    const { title, authorFirstName, authorLastName, publicationYear, publisherName, skip, limit, orderTitle, orderYear } = args
    ...
    if (orderTitle > 1 || orderTitle < -1 || orderYear > 1 || orderYear < -1) throw new Error('orderTitle and orderYear can only be 1 or -1')
    ...
    if (title || authorFirstName || authorLastName || publicationYear || publisherName) {
        const sizeList = db.get().collection('books').find({
            $or: [
                { title: title },
                { authors: author },
        ...
        return db.get().collection('books').find({
            $or: [
                { title: title },
                { authors: author },
                { publisher: publisher._id },
                { publicationYear: publicationYear }]
        }).skip(sizeList > 1 ? skip : 0).limit(limit).toArray()
    }
...
export async function createBook(args) {
    ...
    if (args.publisher.length !== 24) throw new Error('The id must be 24 characters')
...
async function completeFields(book) {
    ...
    if (!book.publicationYear) book['publicationYear'] = existBook.publicationYear
